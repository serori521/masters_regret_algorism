...
# Phase1
@inline function MMRwc_phase1(A::Matrix{T}, k::Int, n::Int)::T where {T<:Real}
    ε = 1e-6 # << 1
...
# Phase2
@inline function MMRwc_phase2(A::Matrix{T}, d̂::T, k::Int, n::Int)::T where {T<:Real}
    ε = 1e-6 # << 1
...
phase3_MMRwc_result = @NamedTuple{
    # 区間重みベクトル
    kwᴸ::Vector{T}, kwᵁ::Vector{T},
} where {T<:Real}

# Phase3_max
@inline function MMRwc_phase3_max(A::Matrix{T}, d̂::T, dd̂::T, k::Int, n::Int)::phase3_MMRwc_result{T} where {T<:Real}
    ε = 1e-6 # << 1
...
# Phase3_min
@inline function MMRwc_phase3_min(A::Matrix{T}, d̂::T, dd̂::T, k::Int, n::Int)::phase3_MMRwc_result{T} where {T<:Real}
    ε = 1e-6 # << 1
...
# 推定法の関数
@inline function MMRwc(A::Matrix{T})::LPResult_Individual{T} where {T<:Real}
    ...
    for k = 1:n
        uwᴸ, uwᵁ = MMRwc_phase3_max(A, d̂[k], dd̂[k], k, n)
        for j = 1:n
            kwᴸ[j, k] = uwᴸ[j]
            kwᵁ[j, k] = uwᵁ[j]
        end
        lwᴸ, lwᵁ = MMRwc_phase3_min(A, d̂[k], dd̂[k], k, n)
        for j = 1:n
            kwᴸ[j, k+n] = lwᴸ[j]
            kwᵁ[j, k+n] = lwᵁ[j]
        end
