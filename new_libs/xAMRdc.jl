...
# Phase3のループの中の部分
@inline function phase3_jump_amrd(A::Matrix{T}, Wᶜ::Matrix{T}, d⃰::T, k::Int, n::Int)::phase3_jump_mmrd_result{T} where {T<:Real}
    ε = 1e-6
    tolerance = 1e-6  # 許容誤差
...
# eAMRd/c法, gAMRd/c法
#@inline function AMRd_kai(A::Matrix{T}, method::Function)::MMRD_Individual_kai{T} where {T<:Real}
@inline function xAMRdc(A::Matrix{T}, method::Function)::LPResult_Individual{T} where {T<:Real}
    if !isCrispPCM(A)
    ...
    for k = 1:n
        (tₖ⃰, L⃰) = phase3_jump_amrd(A, Wᶜ, d⃰[k], k, n)
