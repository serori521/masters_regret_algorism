...
# Phase3のループの中の部分
@inline function phase3_jump_mmrd2(A::Matrix{T}, Wᶜ::Matrix{T}, d⃰::T, k::Int, n::Int)::phase3_jump_mmrd_result2{T} where {T<:Real}
    ε = 1e-6
    tolerance = 1e-6  # 許容誤差
...
# eMMRd/c法, gMMRd/c法(step3の目的関数を変更したバージョン)
@inline function xMMRdc2(A::Matrix{T}, method::Function)::LPResult_Individual{T} where {T<:Real}
    if !isCrispPCM(A)
    ...
    for k = 1:n
        (μₖ⃰, l⃰) = phase3_jump_mmrd2(A, Wᶜ, d⃰[k], k, n)
